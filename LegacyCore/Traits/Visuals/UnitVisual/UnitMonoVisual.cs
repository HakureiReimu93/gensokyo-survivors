...
public partial class UnitMonoVisual : Node2D, IKillable, INewBufSubject
{
	...
	private Adventure DoHitFlashMotion()
	{
		...
		yield return WaitForSeconds(0.25f).Meanwhile((timeLeft, waitTime) =>
						MyWhiteFlashable.Modulate = new Color(1, 1, 1,
						Calculate.PhiEasing(0.7f, (timeLeft / waitTime).Norm10())
						)
		);
		MyWhiteFlashable.Visible = false;
	}
