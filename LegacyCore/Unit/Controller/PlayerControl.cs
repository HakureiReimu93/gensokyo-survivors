...
public partial class PlayerControl : Node, IMobUnitController, IPickupCommandReceiver
{
	...
	private void DoAddNewUpgrade(UpgradeMetaData pUpgrade)
	{
		...
		if (pUpgrade.MyDetails != null)
		{
			if (pUpgrade.MyDetails.TryGetValue(mAttackSpeedPath, out Variant val))
			{
				foreach (var spawner in mSkillRoster.GetChildren().Cast<CooldownAbilitySpawner>())
				{
					spawner.MySpawnDelay = Mathf.Max(1f, spawner.MySpawnDelay - val.AsSingle());
				}
			}
			if (pUpgrade.MyDetails.TryGetValue(mMovementSpeedPath, out Variant val1))
			{
				mUnit.MyMaxSpeed *= val1.AsSingle();
			}
		}
	...
	StringName mMovementSpeedPath = new("player-movement-speed");
	StringName mAttackSpeedPath = new("player-movement-speed");
